...
LOCAL_OBSERVATION_INPUT_SCHEMA_VERSION: Literal["local-observation-discovery-input-v1.1"] = (
    "local-observation-discovery-input-v1.1"
)
LOCAL_OBSERVATION_OUTPUT_SCHEMA_VERSION: Literal["local-observation-discovery-v1.1"] = (
    "local-observation-discovery-v1.1"
)
LOCAL_OBSERVATION_CONTRACT_VERSION: Literal["local-observation-contract-v1.1"] = (
    "local-observation-contract-v1.1"
)
LOCAL_OBSERVATION_ARTIFACT_SCHEMA_VERSION: Literal["model-decision-artifact-v1"] = (
    "model-decision-artifact-v1"
)
LOCAL_OBSERVATION_MODEL_WIRE_SCHEMA_VERSION: Literal["local-observation-model-wire-v1"] = (
    "local-observation-model-wire-v1"
)
LocalObservationPromptVersion = Literal[
...
LOCAL_OBSERVATION_PROMPT_VERSION: Literal["local-observation-discovery-prompt-v1.6"] = (
    "local-observation-discovery-prompt-v1.6"
)
...
class LocalObservationDiscoveryInput(BaseModel):
    """Internal source envelope; the provider sends only chunk_text to M1."""
    ...
    @classmethod
    def reject_duplicate_families(cls, value: list[CoarseVisualFamily]) -> list[CoarseVisualFamily]:
        if len(value) != len(set(value)):
            raise ValueError("duplicate_allowed_coarse_family")
        return value


class LocalObservationModelEntity(BaseModel):
    """Only the entity semantics that M1 must decide."""
...
class LocalObservationModelFact(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class LocalObservationModelTemporalSignal(BaseModel):
    model_config = ConfigDict(extra="forbid")

    owner_index: int | None = Field(default=None, ge=0)
    fact_index: int | None = Field(default=None, ge=0)
    evidence_quote: str = Field(min_length=1)
    signal_kind: TemporalSignalKind


class LocalObservationModelUnresolvedItem(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class LocalObservationModelOutput(BaseModel):
    """Minimal untrusted model wire response before deterministic materialization."""

    model_config = ConfigDict(extra="forbid")

    entities: list[LocalObservationModelEntity] = Field(max_length=128)
    facts: list[LocalObservationModelFact] = Field(max_length=256)
    temporal_signals: list[LocalObservationModelTemporalSignal] = Field(max_length=128)
    unresolved_items: list[LocalObservationModelUnresolvedItem] = Field(max_length=128)

    @model_validator(mode="after")
    def validate_wire_indices(self) -> LocalObservationModelOutput:
        entity_count = len(self.entities)
        fact_count = len(self.facts)
        owner_indices = [item.owner_index for item in self.facts]
        owner_indices.extend(
            item.owner_index for item in self.temporal_signals if item.owner_index is not None
        )
        owner_indices.extend(
            item.owner_index for item in self.unresolved_items if item.owner_index is not None
        )
        if any(index >= entity_count for index in owner_indices):
            raise ValueError("model_wire_owner_index_out_of_range")
        if any(
            item.fact_index is not None and item.fact_index >= fact_count
            for item in self.temporal_signals
        ):
            raise ValueError("model_wire_fact_index_out_of_range")
        for item in self.temporal_signals:
            if item.fact_index is None or item.owner_index is None:
                continue
            if self.facts[item.fact_index].owner_index != item.owner_index:
                raise ValueError("model_wire_signal_owner_mismatch")
        return self
...
class LocalObservationDecisionArtifact(BaseModel):
    ...
    prompt_version: LocalObservationPromptVersion
