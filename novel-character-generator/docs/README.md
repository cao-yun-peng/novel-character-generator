...
| [质量优先语义流水线 V2 契约](27-semantic-pipeline-v2-contract.md) | N0-N11 输入输出、M1-M5 专用语义 Prompt、5 组输入/输出条件 Schema、分组联合复核、Promotion Gate、覆盖/质量指标与 V1 迁移回滚 |
| [M1 局部观察发现测试集审核说明](28-m1-local-observation-evaluation.md) | 第一阶段 M1 的 15 个用户审核 case、评分边界、人工 Gate 与离线评分方法 |
| [M1 首次真实效果基线](29-m1-real-baseline-evaluation.md) | 15-case 真实运行、测量修正、逐类失败归因、成本延迟与 M1 Gate 结论 |
...
- 质量优先语义链重构：27 → 23 → 24 → 25 → 17 → 12 → 13。
