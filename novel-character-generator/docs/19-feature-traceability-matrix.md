...
| TXT 上传与源版本 | `POST /novels`、`POST /novels/{id}/versions` | `IngestionService`、Document ORM、LocalArtifactStore | 创建后续 Run | `test_document_versions.py` | 业务事件待统一 | 已实现 |
...
| 外观状态、冲突、档案 | 角色 appearance/profile/conflict API | `AppearanceService`、Character ORM | 尚无 `aggregate_appearance` | `test_character_appearance_api.py` | 聚合事件尚无 | 部分实现 |
| 目标时点 Snapshot | `GET /characters/{id}/snapshot` | `AppearanceService.snapshot` | — | `test_character_appearance_api.py` | `generation.snapshot.resolved` 待实现 | 已实现核心 |
...
| 外观自动聚合闭环 | 现有查询/审批 API | 目标见[聚合契约](17-appearance-aggregation-contract.md) | `aggregate_appearance` | 尚无 | 聚合事件已定义、未插桩 | 仅设计 |
