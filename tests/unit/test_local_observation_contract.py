...
def test_m1_model_wire_requires_only_semantic_collections() -> None:
    ...
    assert "local_entity_id" not in rendered
    assert "entity_ref" not in rendered


def test_m1_model_wire_rejects_out_of_range_or_mismatched_indices() -> None:
    base: dict[str, object] = {
        "entities": [{"mention_quote": "沈砚", "mention_kind": "explicit_name"}],
        "facts": [],
        "temporal_signals": [],
        "unresolved_items": [],
    }
    ...
    with pytest.raises(ValidationError, match="model_wire_owner_index_out_of_range"):
        LocalObservationModelOutput.model_validate(invalid_owner)
    ...
    with pytest.raises(ValidationError, match="model_wire_fact_index_out_of_range"):
        LocalObservationModelOutput.model_validate(invalid_fact)

    mismatch = {
        **base,
        "entities": [
            {"mention_quote": "沈砚", "mention_kind": "explicit_name"},
    ...
    with pytest.raises(ValidationError, match="model_wire_signal_owner_mismatch"):
        LocalObservationModelOutput.model_validate(mismatch)

    consistent = mismatch.copy()
    consistent["temporal_signals"] = [
        {
            "owner_index": 0,
            "fact_index": 0,
            "evidence_quote": "换了装束",
    ...
    assert LocalObservationModelOutput.model_validate(consistent)
